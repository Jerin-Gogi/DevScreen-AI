    ...
    <div className="h-full overflow-y-auto bg-base-200">
      <div className="p-6 bg-base-100 border-b border-base-300">
        <div className="flex items-start justify-between mb-3">
          ...
          <span
            className={`badge ${getDifficultyBadge(problem.difficulty)} font-semibold`}
          >
            {problem.difficulty}
          </span>
        </div>
        ...
        <div className="mt-4">
          <select
            value={problemId}
            className="select select-sm w-full rounded-2xl cursor-pointer"
            onChange={(e) => onProblemChange(e.target.value)}
          >
            {allProblems.map((each) => {
              return (
                <option
                  key={each.id}
                  value={each.id}
                >{`${each.title} - ${each.difficulty}`}</option>
              );
            })}
          </select>
        </div>
